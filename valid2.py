...
def count_images_in_dataset(root_dir):
    image_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.gif')  # Thêm các định dạng ảnh nếu cần
    total_images = 0
...
def main():
    ...
    model_cfgs = []
    model_names = []
    # Đếm số lượng hình ảnh trong tập dữ liệu
    root_dir = args.data or args.data_dir
    total_images = count_images_in_dataset(root_dir)
    _logger.info(f'Total number of images in the dataset: {total_images}')
